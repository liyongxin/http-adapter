...
//                conn.close();
    ...
    public static String execCommand(String command, ClusterVMDto sshHostInfo){
        Connection connection = login(sshHostInfo.getIp(), sshHostInfo.getUsername(), sshHostInfo.getPassword());
        String result = execute(connection, command);
        return result;
    }
